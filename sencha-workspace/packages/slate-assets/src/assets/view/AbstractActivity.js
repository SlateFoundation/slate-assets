Ext.define('Slate.assets.view.AbstractActivity', {
    extend: 'Ext.Panel',

    xtype: 'abstract-activity-panel',
    requires: [
        'Slate.assets.view.template.Activity'
    ],
    ...
    updateActivityData: function(model) {
        ...
        if (model) {
            Ext.each(model.get('Stories'), function(story) {
                // storyModel = Ext.ModelMgr.create(story, 'Slate.assets.model.Activity');
                storyModel = Slate.assets.model.Activity.create(story);
